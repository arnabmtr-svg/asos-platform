    ...
    if k:
        try:
            ...
            if "NSE:NIFTY 50" in q:
                ...
                out["nifty_chg"] = round(n.get("net_change",0) or (n["last_price"]-n["ohlc"]["close"])/n["ohlc"]["close"]*100,2)
            if "NSE:NIFTY BANK" in q:
                b = q["NSE:NIFTY BANK"]
                out["banknifty"] = b["last_price"]
                out["bnifty_chg"] = round((b["last_price"]-b["ohlc"]["close"])/b["ohlc"]["close"]*100,2)
        except Exception: pass
    return out
